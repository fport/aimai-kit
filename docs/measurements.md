...
**What the models are.** All of these run against deterministic stubs, not
providers. The stubs do real work — the extraction stub parses contracts with
regexes, the selection stub scores queries against tool descriptions, the
harness stub answers only from what is in its context — but they know the
shape of the synthetic data, so their accuracy is optimistic.
